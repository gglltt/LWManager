...
function sign(res, account, alliance){ const payload={accountId:String(account._id),userId:String(account._id),username:account.username,role:account.role,allianceId:account.allianceId??null,allianceCode:alliance?.code||null,serverNumber:alliance?.serverNumber||null,isMaster:account.role==="master"}; const token=jwt.sign(payload,jwtSecret(),{expiresIn:"7d"}); res.cookie("lw_token",token,{httpOnly:true,sameSite:"lax",secure:process.env.NODE_ENV==="production",maxAge:7*24*60*60*1000}); }
...
router.post("/login", async (req,res)=>{ const p=pin(req.body.pin); const codeRaw=String(req.body.allianceCode||"").trim(); const isMaster=codeRaw.toLowerCase()==="master"; const ip=req.ip||"unknown"; const t = res.locals.t || ((k) => k); const generic="Credenziali non valide."; let tenantKey="master"; let alliance=null;
  ...
  if(isMaster){ if(isBlocked(ip, tenantKey)) return res.status(429).render("login",{error:t("too_many_attempts"),message:null,form:req.body}); const account=await Account.findOne({username:"master",role:"master",isActive:true}).lean(); const ok=account && await bcrypt.compare(p, account.pinHash || account.passwordHash || ""); if(!ok){ addFail(ip, tenantKey); await createEventLog(req,"login_failed",""); return res.status(401).render("login",{error:generic,message:null,form:req.body}); } clearFail(ip, tenantKey); sign(res, account, null); req.user={accountId:String(account._id),username:account.username,role:"master",isMaster:true,allianceId:null,allianceCode:null,serverNumber:null}; await createEventLog(req,"login_success","role=master"); return res.redirect("/dashboard"); }
  ...
  if(isBlocked(ip, tenantKey)){ await createEventLog(req,"login_rate_limited",""); return res.status(429).render("login",{error:t("too_many_attempts"),message:null,form:req.body}); }
  ...
  const accounts=await Account.find({allianceId:alliance.allianceId,isActive:true,role:{$in:["supervisor","standard"]}}).lean();
  let matched=null; for(const a of accounts){ if(await bcrypt.compare(p, a.pinHash || a.passwordHash || "")){ matched=a; break; } }
  ...
  clearFail(ip, tenantKey); sign(res, matched, alliance); req.user={accountId:String(matched._id),username:matched.username,role:matched.role,allianceId:alliance.allianceId,allianceCode:alliance.code,serverNumber:alliance.serverNumber,isMaster:false}; await createEventLog(req,"login_success",`role=${matched.role}`); res.redirect("/dashboard"); });

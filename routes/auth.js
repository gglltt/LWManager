...
router.post("/login", async (req,res)=>{ const p=pin(req.body.pin); const codeRaw=String(req.body.allianceCode||"").trim(); const isMaster=codeRaw.toLowerCase()==="master"; const ip=req.ip||"unknown"; const t = res.locals.t || ((k) => k); const generic=t("invalid_credentials"); let tenantKey="master"; let alliance=null;
